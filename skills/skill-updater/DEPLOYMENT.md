...
4. Tell the user that `-Apply` assists only the configured user-level subset without a backup, `-Overwrite` is separately required for different local content or lock metadata, `-Prune` is separately required to remove source-owned entries outside the configured allowlist, and `-PurgeLegacy` is separately required to remove unprotected contents from the deprecated `~/.codex/skills` root. Before purge, show exact candidates and protected paths; report any symlink/Junction and leave it untouched. Repository, admin/system, legacy, and plugin scopes are read-only inventory surfaces. The report and task-only repair paths do not change skills.
...
Run `scripts/Deploy.ps1` from Windows PowerShell. Add `-PurgeLegacy` when the reviewed run should also remove unprotected contents from the deprecated `~/.codex/skills` root. The scripts resolve the clone path and current user at runtime, so copying a task definition from another PC is invalid. Register the task for the current interactive user with limited privileges; it should start when available and should not wake the PC.
...
- no divergent same-name entry remains under `~/.codex/skills`; unprotected legacy entries are absent after `-PurgeLegacy`, while `.system`, `codex-primary-runtime`, and reparse-point entries remain protected;
